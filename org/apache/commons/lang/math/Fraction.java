 ...
 * @since 2.0
 * @version $Id: Fraction.java,v 1.1 2008/07/14 19:24:35 rlexvold Exp $
 */
public final class Fraction extends Number implements Serializable, Comparable
{
	...
	public static Fraction getFraction(int numerator, int denominator)
	{
		...
		if (denominator < 0)
		{
			if (numerator == Integer.MIN_VALUE || denominator == Integer.MIN_VALUE) throw new ArithmeticException("overflow: can't negate");
			numerator = -numerator;
	...
	/**
	 * <p>
	 * Creates a <code>Fraction</code> instance with the 3 parts of a fraction
	 * X Y/Z.
	 * </p>
	 * 
	 * <p>
	...
	public static Fraction getFraction(int whole, int numerator, int denominator)
	{
		...
		else numeratorValue = whole * (long) denominator + numerator;
		if (numeratorValue < Integer.MIN_VALUE || numeratorValue > Integer.MAX_VALUE) throw new ArithmeticException("Numerator too large to represent as an Integer.");
	...
	public static Fraction getFraction(double value)
	{
		...
		if (value > Integer.MAX_VALUE || Double.isNaN(value)) throw new ArithmeticException("The value must not be greater than Integer.MAX_VALUE or NaN");
	...
	{
		if (str == null) throw new IllegalArgumentException("The string must not be null");
		// parse double format
		int pos = str.indexOf('.');
	...
	/**
	 * <p>
	 * Gets the fraction as an <code>int</code>. This returns the whole
	...
	/**
	 * <p>
	 * Gets the fraction as a <code>long</code>. This returns the whole
	 * number part of the fraction.
	 * </p>
	 * 
	 * @return the whole number fraction part
	...
	/**
	 * <p>
	 * Gets the fraction as a <code>float</code>. This calculates the
	...
	/**
	 * <p>
	 * Gets the fraction as a <code>double</code>. This calculates the
	 * fraction as the numerator divided by denominator.
	 * </p>
	 * 
	...
	public Fraction invert()
	{
		...
		else return new Fraction(denominator, numerator);
	}
	 ...
	 * @return <code>this</code> if the power is one, <code>ONE</code> if
	 *         the power is zero (even if the fraction equals ZERO) or a new
	...
	public Fraction pow(int power)
	{
		...
		{
			if (power == Integer.MIN_VALUE) return this.invert().pow(2).pow(-(power / 2));
	...
	public int hashCode()
	{
		if (hashCode == 0) // hashcode update should be atomic.
	...
	{
		if (toString == null) toString = new StringBuffer(32).append(getNumerator()).append('/').append(getDenominator()).toString();
		return toString;
	}
	 ...
	 * If the whole number is zero it will be ommitted. If the numerator is
	...
	{
		if (toProperString == null) if (numerator == 0)
			toProperString = "0";
		...
		else if ((numerator > 0 ? -numerator : numerator) < -denominator)
		{
			...
			else toProperString = new StringBuffer(32).append(getProperWhole()).append(' ').append(properNumerator).append('/').append(getDenominator()).toString();
		} else toProperString = new StringBuffer(32).append(getNumerator()).append('/').append(getDenominator()).toString();
