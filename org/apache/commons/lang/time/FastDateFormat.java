 ...
 * <code>SimpleDateFormat</code> is not thread-safe in any JDK version, nor
 ...
 * 
 * <p>
 * Java 1.4 introduced a new pattern letter, <code>'Z'</code>, to represent
 * time zones in RFC822 format (eg. <code>+0800</code> or <code>-1100</code>).
 * This pattern letter can be used here (on all JDK versions).
 * </p>
 * 
 * <p>
 * In addition, the pattern <code>'ZZ'</code> has been made to represent
 * ISO8601 full format time zones (eg. <code>+08:00</code> or
 * <code>-11:00</code>). This introduces a minor incompatibility with Java
 ...
 * @since 2.0
 * @version $Id: FastDateFormat.java,v 1.1 2008/07/14 19:24:35 rlexvold Exp $
 */
public class FastDateFormat extends Format
{
	...
	{
		if (cDefaultPattern == null) cDefaultPattern = new SimpleDateFormat().toPattern();
	...
	{
		super();
		if (pattern == null) throw new IllegalArgumentException("The pattern must not be null");
		mPattern = pattern;
		...
		for (int i = 0; i < length; i++)
		{
			...
			switch (c)
			{
				...
				case 'y': // year (number)
					...
					else rule = TwoDigitYearField.INSTANCE;
					break;
				case 'M': // month in year (text and number)
					...
					else rule = UnpaddedMonthField.INSTANCE;
					break;
				...
				case 'z': // time zone (text)
					...
					else rule = new TimeZoneNameRule(mTimeZone, mTimeZoneForced, mLocale, TimeZone.SHORT);
					break;
				case 'Z': // time zone (value)
					if (tokenLen == 1)
						rule = TimeZoneNumberRule.INSTANCE_NO_COLON;
					else rule = TimeZoneNumberRule.INSTANCE_COLON;
					break;
				case '\'': // literal text
					String sub = token.substring(1);
					if (sub.length() == 1)
						rule = new CharacterLiteral(sub.charAt(0));
					else rule = new StringLiteral(sub);
					break;
				default:
	...
	/**
	 * <p>
	 * Formats a <code>Date</code>, <code>Calendar</code> or
	 * <code>Long</code> (milliseconds) object.
	 * </p>
	 * 
	 * @param obj
	...
	public StringBuffer format(Object obj, StringBuffer toAppendTo, FieldPosition pos)
	{
		...
		else throw new IllegalArgumentException("Unknown class: " + (obj == null ? "<null>" : obj.getClass().getName()));
	}
	 ...
	 * <code>Calendar</code> is passed in to be formatted, the time zone on
	 * that may be used depending on {@link #getTimeZoneOverridesCalendar()}.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Returns <code>true</code> if the time zone of the calendar overrides
	 * the time zone of the formatter.
	 * </p>
	 * 
	...
	public boolean equals(Object obj)
	{
		...
		if ((mPattern == other.mPattern || mPattern.equals(other.mPattern)) && (mTimeZone == other.mTimeZone || mTimeZone.equals(other.mTimeZone))
				&& (mLocale == other.mLocale || mLocale.equals(other.mLocale)) && (mTimeZoneForced == other.mTimeZoneForced) && (mLocaleForced == other.mLocaleForced))
			return true;
		return false;
	}
	...
	private static class PaddedNumberField implements NumberRule
	{
		...
		{
			if (size < 3) // Should use UnpaddedNumberField or
							// TwoDigitNumberField.
		...
		public final void appendTo(StringBuffer buffer, int value)
		{
			...
			} else
			{
				...
				if (value < 1000)
					digits = 3;
				else digits = (int) (Math.log(value) / LOG_10) + 1;
	...
	private static class TimeZoneNameRule implements Rule
	{
		...
		public void appendTo(StringBuffer buffer, Calendar calendar)
		{
			if (mTimeZoneForced)
			{
				...
				else buffer.append(mStandard);
			} else
			{
				TimeZone timeZone = calendar.getTimeZone();
				if (timeZone.useDaylightTime() && calendar.get(Calendar.DST_OFFSET) != 0)
					buffer.append(getTimeZoneDisplay(timeZone, true, mStyle, mLocale));
				else buffer.append(getTimeZoneDisplay(timeZone, false, mStyle, mLocale));
			}
		}
	}
	/**
