/* vim:set softtabstop=3 shiftwidth=3 tabstop=3 expandtab tw=72:
   $Id: Rebuilder.java,v 1.2 2008/10/22 06:26:48 rlexvold Exp $
...
public class Rebuilder
{
	 ...
	 * 
	 * @param out
	 *            The sink for reconstructed data.
	 * @param buf
	 *            The original data.
	 * @param deltas
	 *            The deltas to apply.
	 */
	public static void rebuild(OutputStream out, byte[] buf, List deltas) throws IOException
	{
		...
		for (int i = 0; i < darray.length; i++)
		{
			...
			{
				out.write(buf, (int) ((Offsets) darray[i]).getOldOffset(), darray[i].getBlockLength());
			}
		}
	}
	 ...
	 *            The deltas to apply.
	 */
	public static void rebuild(OutputStream out, File oldFile, List deltas) throws IOException
	{
	...
	/**
	 * Reconstruct a file into a new file created with
	 * {@link java.io.File#createTempFile(java.lang.String,java.lang.String,java.io.File)}
	 * . This file can then be renamed to the destination.
	 * 
	 * @param oldFile
	 *            The original file.
	 * @param deltas
	 *            A collection of {@link Delta}s to apply to the original file.
	 * @return A unique {@link java.io.File} containing the reconstruction.
	 */
	public static File rebuildFile(File oldFile, List deltas) throws IOException
	{
		File newFile = File.createTempFile(TMP_PREFIX, TMP_SUFFIX, oldFile.getParentFile());
	 ...
	 *            The original file.
	 * @param newFile
	 *            The file to write the reconstruction to. This must be a
	 *            different file than <code>oldFile</code>
	 * @param deltas
	 *            The {@link Delta}s to apply.
	 */
	 ...
	 *            The {@link Delta}s to apply.
	 */
	public static void rebuildFileInPlace(File file, List deltas) throws IOException
	{
		...
		for (Iterator i = deltas.iterator(); i.hasNext();)
		{
			...
			{
				dataBlocks.add(o);
				newFileLength = Math.max(newFileLength, ((DataBlock) o).getOffset() + ((DataBlock) o).getBlockLength());
			}
		}
		if (copyOnly)
		{
	...
	{
		return (o1.getNewOffset() >= o2.getOldOffset() && o1.getNewOffset() <= o2.getOldOffset() + o2.getBlockLength())
				|| (o1.getNewOffset() + o1.getBlockLength() >= o2.getOldOffset() && o1.getNewOffset() + o1.getBlockLength() <= o2.getOldOffset() + o2.getBlockLength());
	}
	// Private inner classes.
	// -----------------------------------------------------------------------
	/**
	 * <p>
	 * This class topologically sorts a directed graph encoded as a mapping from
	 * vertices to sets of adjacent vertices. It is used when reconstructing a
	 * file in-place, where it is possible that Offsets objects overlap.
	 * </p>
	 * 
	 * <p>
	 * Since a topological ordering of a directed graph is only possible in
	 ...
	 * ordering can be obtained with {@link #getFinished()}, which is a
	 * {@link java.util.List} of nodes in their sorted order. Nodes that were
	 * part of cycles but have been removed can be retrieved with
	 * {@link #getCycleNodes()}, which returns a {@link java.util.Set} of nodes
	 * that were removed.
	 * </p>
	 * 
	 * <p>
	 * Despite the terminology of "removed" nodes, the graph passed to the
	...
	private static class TopologicalSorter
	{
		...
		/**
		 * The graph, encoded as a mapping between nodes and
		 * {@link java.util.Set}s of adjacent nodes.
		 */
		private Map					graph;
		/**
		...
		/**
		 * <p>
		 * Depth-first search with removing nodes with back edges. As this
		 * method runs it places nodes into a list, which is a topological
		 * sorting of this graph. Any node encountered with a back-edge is put
		 * into the cycle-nodes list.
		 * </p>
		 * 
		 * <p>
		 * References:
