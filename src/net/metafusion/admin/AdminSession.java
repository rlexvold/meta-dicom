...
import net.metafusion.gui.GUIAdminClient;
import net.metafusion.gui.MainFrame;
...
import acme.util.swing.PasswordDialog;
...
public class AdminSession
{
	...
	public AdminClient getClient(String host, int port)
	{
		return new GUIAdminClient(host, port);
	}

	public AdminClient getCurrentClient()
	{
	...
	// hack -- for net access via external address
	public AdminClient getCurrentClient(String xhost, int xport)
	{
		if (MainFrame.getFrame() == null)
			return new AdminClient(host, port);
		else
	...
	{
		List fileNames = new AdminClient(host, port).getOlderConfigs();
	...
	public boolean loadConfig(String host, int port, String name)
	{
		...
		try
		{
			boolean established = getClient(host, port).establish(null);
			while (!established)
			{
				String password = PasswordDialog.getPasswordFromDialog();
				if (password == null)
					throw new RuntimeException("no password entered");
				established = getClient(host, port).establish(password);
				if (!established)
					JOptionPane.showMessageDialog(null, "Invalid Password", "Error", JOptionPane.WARNING_MESSAGE);
			}
			String adminString = getClient(host, port).getConfig(name);
			Log.log("loadConfig:" + adminString);
			XML x = XML.parseXML(adminString);
			...
			configXML = x;
			expiration = getClient(host, port).getExpiration();
			version = getClient(host, port).getVersion();
		}
		catch (Exception e)
		{
			Util.log("could not load config: " + e);
	...
	private boolean storeConfig(String host, int port, XML config) throws Exception
	{
		if (config == nullXML)
			return true;
		Date d = new Date();
		config.getNode("admin").addAttr("date", d.toString());
		getClient(host, port).saveAdmin(config.toString());
	...
	public void addAE(AEBean ae)
	{
		...
		if (aeMap.containsKey(ae.getName().toUpperCase()))
	...
	{
		// Util.log(""+aeMap.keySet());
		if (!aeMap.containsKey(ae.getName().toUpperCase()))
			throw new RuntimeException("AE " + ae.getName() + " does not exist.");
		aeMap.remove(ae.getName().toUpperCase());
		aeList.remove(ae);
	...
	{
		boolean fail = true;
		AEBean orig = (AEBean) aeMap.get(ae.getName().toUpperCase());
		removeAE(ae);
		try
		{
			ae.setHost(host);
	...
	public boolean verifyAE(String ae)
	{
		...
		try
		{
			result = getClient(host, port).verify(ae);
	...
	public StorageBean getStorage(ServerBean sb)
	{
		...
		try
		{
			...
			// long[] l = getClient(ae.getHost(),
			// Integer.parseInt(ae.getPort())).getCapacityFreeSpace();
			// String[] s = getClient(ae.getHost(),
			// Integer.parseInt(ae.getPort())).getFreePerUsedSpace();
			String[] s = getCurrentClient(ae.getHost(), Integer.parseInt(ae.getPort())).getFreePerUsedSpace();
			stb = new StorageBean(s);
			sb.setInvalid(false);
		}
		catch (Exception e)
		{
	...
	public boolean saveLog(ServerBean serverBean, File f)
	{
		AEBean aeBean = getAE(serverBean.getAE());
		return getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort())).getLogFile(f);
		// return getClient(aeBean.getHost(),
		// Integer.parseInt(aeBean.getPort())).getLogFile(f);
	}
	...
	// XML x = new XML(""); // AdminClient.doLog(aeBean.getHost(),
	...
	public StudyBean[] search(String ae, SearchBean search)
	{
		...
		// List al = getClient(aeBean.getHost(),
		// Integer.parseInt(aeBean.getPort())).search(search);
		List al = getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort())).search(search);
	...
	public String[] send(String sourceAE, List studyBeans, String ae)
	{
		...
		for (Iterator iter = studyBeans.iterator(); iter.hasNext();)
		{
			...
			if (sb.isSelected())
				studies.add(sb.getStudyUID());
		}
		AdminClient client = getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort()));
		boolean good = client.sendStudyList(ae, studies);
		// boolean good = getClient(aeBean.getHost(),
		// Integer.parseInt(aeBean.getPort())).sendStudyList(ae, studies);
		if (good)
			return (String[]) studies.toArray(new String[0]);
		else
			return new String[0];
		//
		//
	...
	public String[] delete(String sourceAE, List studyBeans)
	{
		...
		List deleteList = getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort())).deleteStudyList(studies);
		return (String[]) deleteList.toArray(new String[0]);
		//
		//
		// AEBean aeBean = this.getAE(sourceAE);
		// XML studies = new XML("studies");
	...
	public StudiesInfoBean getInfo(String sourceAE, List studyBeans)
	{
		...
		StudiesInfoBean sib = getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort())).getStudyListInfo(studies);
		return sib;
		// return (String[])deleteList.toArray(new String[0]);
		//
		// AEBean aeBean = this.getAE(sourceAE);
	...
	public StudiesInfoBean burnStudyList(String sourceAE, List studyBeans)
	{
		AEBean aeBean = this.getAE(sourceAE);
		List studies = new ArrayList();
		for (Iterator iter = studyBeans.iterator(); iter.hasNext();)
		...
		StudiesInfoBean sib = getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort())).burnStudyList(studies);
		return sib;
	}
	...
	public boolean archive(String sourceAE, List studyBeans, File rootDir)
	{
		...
		try
		{
			...
			good = getCurrentClient(aeBean.getHost(), Integer.parseInt(aeBean.getPort())).archiveStudyList(studies, f);
			if (good)
