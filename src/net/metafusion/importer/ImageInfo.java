...
public class ImageInfo
{
	...
	private boolean checkJpeg() throws IOException
	{
		...
		while (true)
		{
			...
			{ // APPx
				...
				if (equals(APP0_ID, 0, data, 0, 5)) // System.out.println("data 7=" + data[7]);
					if (data[7] == 1)
					{
			...
			else if (marker >= 0xffc0 && marker <= 0xffcf && marker != 0xffc4 && marker != 0xffc8)
			{
				if (read(data, 0, 6) != 6)
					return false;
				format = FORMAT_JPEG;
				samplesPerPixel = data[5] & 0xff;
				bitsPerSample = data[0] & 0xff;
	...
	private boolean checkPcx() throws IOException
	{
		...
		if (planes == 1 && (bits == 1 || bits == 2 || bits == 4 || bits == 8))
			// paletted
			bitsPerPixel = bits;
		else if (planes == 3 && bits == 8) // RGB truecolor
			bitsPerPixel = 24;
		else
			return false;
