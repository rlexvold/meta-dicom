...
public class XmlRpcClient extends XmlRpcParser implements XmlRpcInvocationHandler
{
	...
	private void beginCall(String methodName) throws XmlRpcException
	{
		try
		{
			...
			{
				openConnection();
				writer = new BufferedWriter(new OutputStreamWriter(connection.getOutputStream(), XmlRpcMessages.get().getString("XmlRpcClient.Encoding")));
			}
			else
			{
			...
			writer.write(XmlRpcMessages.get().getString("XmlRpcClient.Encoding"));
			writer.write("\"?>");
		...
		{
			throw new XmlRpcException(XmlRpcMessages.get().getString("XmlRpcClient.NetworkError"), ioe);
		}
	}

	/**
	 * Finalizes the XML buffer to be sent to the server, and creates a HTTP
	...
	private void handleResponse() throws XmlRpcFault
	{
		...
		{
			throw new XmlRpcException(XmlRpcMessages.get().getString("XmlRpcClient.ParseError"), e);
		}
		if (isFaultResponse)
		{
	...
	private void openConnection() throws IOException
	{
		...
		connection.setRequestProperty("Content-Type", "text/xml; charset=" + XmlRpcMessages.get().getString("XmlRpcClient.Encoding"));
