 ...
 * contained a fault message. If a call could not be made due to a local problem
 * (if an argument could not be serialized or if there was a network problem) an
 * XmlRpcException is thrown instead.
 * 
...
public class XmlRpcFault extends Exception
{
	...
	/**
	 * Creates a new exception with the supplied message and error code. The
	 * message and error code values are those returned from the remote XML-RPC
	 * service.
	 * 
	 * @param message
