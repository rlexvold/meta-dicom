 ...
 * @author Phil Steitz
 * @since 1.0
 * @version $Id: NumberUtils.java,v 1.1 2008/07/14 19:24:33 rlexvold Exp $
 * 
...
public final class NumberUtils
{
	...
	/**
	 * <p>
	 * <code>NumberUtils</code> instances should NOT be constructed in
	 ...
	 * 
	 * <p>
	 * First, the value is examined for a type qualifier on the end (<code>'f','F','d','D','l','L'</code>).
	 * If it is found, it starts trying to create successively larger types from
	 ...
	 * 
	 * <p>
	 * If the string starts with <code>0x</code> or <code>-0x</code>, it
	 * will be interpreted as a hexadecimal integer. Values with leading
	 * <code>0</code>'s will not be interpreted as octal.
	 * </p>
	 * 
	 * @param val
	...
	public static Number createNumber(String val) throws NumberFormatException
	{
		...
		if (val.length() == 0) throw new NumberFormatException("\"\" is not a valid number.");
		if (val.startsWith("--")) // this is protection for poorness in
		...
		if (decPos > -1)
		{
			if (expPos > -1)
			{
				if (expPos < decPos) throw new NumberFormatException(val + " is not a valid number.");
			...
			switch (lastChar)
			{
				...
				case 'F':
					try
					{
						...
						if (!(f.isInfinite() || (f.floatValue() == 0.0F && !allZeros))) // If
																						// it's
																						// too
																						// big
																						// for
	...
	/**
	 * <p>
	 * Convert a <code>String</code> to a <code>Integer</code>, handling
	...
	public static boolean isNumber(String str)
	{
		...
		if (sz > start + 1) if (chars[start] == '0' && chars[start + 1] == 'x')
		{
			int i = start + 2;
		...
		while (i < sz || (i < sz + 1 && allowSigns && !foundDigit))
		{
			...
			{
				if (hasDecPoint || hasExp) // two decimal points or dec in
											// exponent
					return false;
				hasDecPoint = true;
		...
		if (i < chars.length)
		{
			if (chars[i] >= '0' && chars[i] <= '9') // no type qualifier, OK
				return true;
			if (chars[i] == 'e' || chars[i] == 'E') // can't have an E at the
													// last byte
				return false;
			if (!allowSigns && (chars[i] == 'd' || chars[i] == 'D' || chars[i] == 'f' || chars[i] == 'F')) return foundDigit;
			if (chars[i] == 'l' || chars[i] == 'L') // not allowing L with an
													// exponent
