 ...
 * <li><b>ContainsOnly/ContainsNone</b> - does String contains only/none of
 * these characters</li>
 ...
 * <li><b>Difference</b> - compares two Strings and reports on their
 * differences</li>
 ...
 * @since 1.0
 * @version $Id: StringUtils.java,v 1.1 2008/07/14 19:24:33 rlexvold Exp $
 */
public class StringUtils
{
	...
	public static String stripStart(String str, String stripChars)
	{
		...
		if (stripChars == null)
			...
			return str;
		else while ((start != strLen) && (stripChars.indexOf(str.charAt(start)) != -1))
			start++;
	...
	public static String stripEnd(String str, String stripChars)
	{
		...
		if (stripChars == null)
			...
			return str;
		else while ((end != 0) && (stripChars.indexOf(str.charAt(end - 1)) != -1))
			end--;
	...
	{
		int strsLen;
		if (strs == null || (strsLen = strs.length) == 0) return strs;
	...
	{
		if (isEmpty(str) || ArrayUtils.isEmpty(searchChars)) return -1;
		for (int i = 0; i < str.length(); i++)
		{
			char ch = str.charAt(i);
			for (char element : searchChars)
				if (element == ch) return i;
		}
		return -1;
	}
	...
	{
		if (isEmpty(str) || isEmpty(searchChars)) return -1;
	...
	public static String substring(String str, int start)
	{
		...
		if (start < 0) start = str.length() + start; // remember start is
														// negative
		if (start < 0) start = 0;
	...
	public static String[] splitByWholeSeparator(String str, String separator, int max)
	{
		...
		if ((separator == null) || ("".equals(separator))) // Split on
															// whitespace.
		...
		while (end < len)
		{
			...
			if (end > -1)
			{
				...
				} else // We found a consecutive occurrence of the separator,
						// so skip it.
				beg = end + separatorLength;
			} else
			{
	 ...
	 * line with the deprecated <code>isSpace</code> method.
	 * </p>
	 * 
	 * @param str
	...
	public static String deleteWhitespace(String str)
	{
		...
		int count = 0;
		for (int i = 0; i < sz; i++)
			if (!Character.isWhitespace(str.charAt(i))) chs[count++] = str.charAt(i);
	...
	public static String replaceChars(String str, String searchChars, String replaceChars)
	{
		...
		for (int i = 0; i < str.length(); i++)
		{
			...
			{
				modified = true;
				if (index < replaceChars.length()) buf.append(replaceChars.charAt(index));
			} else buf.append(ch);
		}
		if (modified)
			return buf.toString();
	...
	/**
	 * <p>
	 * Removes <code>separator</code> from the end of <code>str</code> if
	 * it's there, otherwise leave it alone.
	 * </p>
	 * 
	 * <p>
	...
	public static String chop(String str)
	{
		...
		char last = str.charAt(lastIdx);
		if (last == '\n') if (ret.charAt(lastIdx - 1) == '\r') return ret.substring(0, lastIdx - 1);
		return ret;
	}
	...
	private static String padding(int repeat, char padChar)
	{
		...
		if (pad == null) pad = String.valueOf(padChar);
	...
	public static boolean isAlpha(String str)
	{
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (Character.isLetter(str.charAt(i)) == false) return false;
		return true;
	}
	...
	public static boolean isAlphaSpace(String str)
	{
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if ((Character.isLetter(str.charAt(i)) == false) && (str.charAt(i) != ' ')) return false;
		return true;
	}
	 ...
	 * @param str
	 *            the string to check, may be null
	 * @return <code>true</code> if every character is in the range 32 thru
	 *         126
	 * @since 2.1
	 */
	public static boolean isAsciiPrintable(String str)
	{
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (CharUtils.isAsciiPrintable(str.charAt(i)) == false) return false;
		return true;
	}
	...
	public static String reverseDelimitedString(String str, String separatorChars)
	{
		...
		ArrayUtils.reverse(strs);
		if (separatorChars == null) return join(strs, ' ');
	...
	public static String abbreviate(String str, int offset, int maxWidth)
	{
		...
		if ((offset + (maxWidth - 3)) < str.length()) return "..." + abbreviate(str.substring(offset), maxWidth - 3);
